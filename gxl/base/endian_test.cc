...
TEST(EndianessTest, ghtonll_gntohll) {
  // Test that gxl::ghtonl compiles correctly
  uint32_t test = 0x01234567;
  EXPECT_EQ(gxl::gntohl(gxl::ghtonl(test)), test);

  uint64_t comp = gxl::ghtonll(kInitialNumber);
  EXPECT_EQ(comp, kInitialInNetworkOrder);
  comp = gxl::gntohll(kInitialInNetworkOrder);
  ...
  uint64_t value = 1;
  for (int i = 0; i < 100; ++i) {
    comp = gxl::ghtonll(gxl::gntohll(value));
    EXPECT_EQ(value, comp);
    comp = gxl::gntohll(gxl::ghtonll(value));
    EXPECT_EQ(value, comp);
    value *= 37;
  }
}
