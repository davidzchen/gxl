...
TEST(StatusBuilder, OkStatusLvalue) {
  StatusBuilder builder(absl::OkStatus(), GXL_LOC);
...
TEST(StatusBuilder, OkStatusRvalue) {
  absl::Status status = StatusBuilder(absl::OkStatus(), GXL_LOC)
                        << "annotated message1 "
                        << "annotated message2";
  ASSERT_EQ(status, absl::OkStatus());
}
...
TEST(StatusBuilder, AppendModeLvalue) {
  StatusBuilder builder(
      absl::Status(absl::StatusCode::kInternal, "original message"), GXL_LOC);
