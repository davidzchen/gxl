...
#ifndef GXL_STATUS_STATUS_BUILDER_H_
#define GXL_STATUS_STATUS_BUILDER_H_
...
class ABSL_MUST_USE_RESULT StatusBuilder {
 public:
  ...
  // occurs.  A typical user will call this with `GXL_LOC`.
  StatusBuilder(const absl::Status& original_status,
                gxl::source_location location)
      : impl_(original_status.ok()
                  ? nullptr
                  : std::make_unique<Impl>(original_status, location)) {}

  StatusBuilder(absl::Status&& original_status, gxl::source_location location)
      : impl_(original_status.ok()
                  ? nullptr
  ...
  // occurs.  A typical user will call this with `GXL_LOC`.
  StatusBuilder(absl::StatusCode code, gxl::source_location location)
 ...
 private:
  struct Impl {
    ...
    Impl(const absl::Status& status, gxl::source_location location);
    Impl(absl::Status&& status, gxl::source_location location);
    Impl(const Impl&);
...
inline StatusBuilder AlreadyExistsErrorBuilder(gxl::source_location location) {
...
inline StatusBuilder InternalErrorBuilder(gxl::source_location location) {
...
inline StatusBuilder NotFoundErrorBuilder(gxl::source_location location) {
  return StatusBuilder(absl::StatusCode::kNotFound, location);
}

inline StatusBuilder UnavailableErrorBuilder(gxl::source_location location) {
  return StatusBuilder(absl::StatusCode::kUnavailable, location);
}

inline StatusBuilder UnimplementedErrorBuilder(gxl::source_location location) {
  return StatusBuilder(absl::StatusCode::kUnimplemented, location);
}

inline StatusBuilder UnknownErrorBuilder(gxl::source_location location) {
  return StatusBuilder(absl::StatusCode::kUnknown, location);
}

}  // namespace gxl

#endif  // GXL_STATUS_STATUS_BUILDER_H_
