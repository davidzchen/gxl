...
#ifndef GXL_FILE_PATH_H_
#define GXL_FILE_PATH_H_
...
// You can get a well-formed path by using gxl::CleanPath(), and
...
//
// Usage:
// string path = gxl::JoinPath("/cns", dirname, filename);
// string path = gxl::JoinPath(FLAGS_test_srcdir, filename);
//
...
// This is the behavior of the old gxl::JoinPath().
// Arguments must be convertible to absl::string_view.
//
// Usage:
// string path = gxl::JoinPathRespectAbsolute("/cns", dirname, filename);
// string path = gxl::JoinPathRespectAbsolute(FLAGS_test_srcdir, filename);
template <typename... T>
...
// there is no "." in the basename, this is equivalent to gxl::Basename(path).
