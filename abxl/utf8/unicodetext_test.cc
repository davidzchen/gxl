...
TEST_F(IteratorTest, Encode) {
  ...
  EXPECT_TRUE(memcmp(text_.utf8_data(), utf8.data(), text_.utf8_length()) == 0);
  ...
  text_.push_back(U'\xAE');  // registered trademark sign
...
TEST_F(IteratorTest, Decode) {
  ...
  std::string s = CodepointString(text_);
