...
#ifndef ABXL_STATUS_BUILDER_H_
#define ABXL_STATUS_BUILDER_H_
...
#include "abxl/base/source_location.h"
...
class ABSL_MUST_USE_RESULT StatusBuilder {
 public:
  ...
  // occurs.  A typical user will call this with `ABXL_LOC`.
  StatusBuilder(const absl::Status& original_status,
                abxl::source_location location)
      : impl_(original_status.ok()
                  ? nullptr
                  : std::make_unique<Impl>(original_status, location)) {}

  StatusBuilder(absl::Status&& original_status, abxl::source_location location)
      : impl_(original_status.ok()
                  ? nullptr
  ...
  // will use `location` as the location from which the log message
  // occurs.  A typical user will call this with `ABXL_LOC`.
  StatusBuilder(absl::StatusCode code, abxl::source_location location)
 ...
 private:
  struct Impl {
    ...
    Impl(const absl::Status& status, abxl::source_location location);
    Impl(absl::Status&& status, abxl::source_location location);
    Impl(const Impl&);
...
inline StatusBuilder AlreadyExistsErrorBuilder(abxl::source_location location) {
...
inline StatusBuilder InternalErrorBuilder(abxl::source_location location) {
...
inline StatusBuilder NotFoundErrorBuilder(abxl::source_location location) {
  return StatusBuilder(absl::StatusCode::kNotFound, location);
}

inline StatusBuilder UnavailableErrorBuilder(abxl::source_location location) {
  return StatusBuilder(absl::StatusCode::kUnavailable, location);
}

inline StatusBuilder UnimplementedErrorBuilder(abxl::source_location location) {
  return StatusBuilder(absl::StatusCode::kUnimplemented, location);
}

inline StatusBuilder UnknownErrorBuilder(abxl::source_location location) {
  return StatusBuilder(absl::StatusCode::kUnknown, location);
}

}  // namespace abxl

#endif  // ABXL_STATUS_BUILDER_H_
