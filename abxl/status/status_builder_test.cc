...
TEST(StatusBuilder, OkStatusLvalue) {
  StatusBuilder builder(absl::OkStatus(), ABXL_LOC);
...
TEST(StatusBuilder, OkStatusRvalue) {
  absl::Status status = StatusBuilder(absl::OkStatus(), ABXL_LOC)
                        << "annotated message1 "
                        << "annotated message2";
  ASSERT_EQ(status, absl::OkStatus());
}
...
TEST(StatusBuilder, AppendModeLvalue) {
  StatusBuilder builder(
      absl::Status(absl::StatusCode::kInternal, "original message"), ABXL_LOC);
