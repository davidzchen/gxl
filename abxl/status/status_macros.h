...
//   }
//
// The macro ends with a `abxl::StatusBuilder` which allows the returned
...
//   }
//
// `abxl::StatusBuilder` supports adapting the builder chain using a
...
// interfaces.  See `abxl::TaskReturn` and `rpc::RpcSetStatus` for
// details.
//
//   void Read(StringPiece name, abxl::Task* task) {
//     int64 id;
...
// to avoid confusion between a `abxl::StatusBuilder` and a
...
#define RETURN_IF_ERROR(expr)                                   \
  ...
  if (abxl::status_macro_internal::StatusAdaptorForMacros       \
          status_macro_internal_adaptor = {(expr), ABXL_LOC}) { \
  } else /* NOLINT */                                           \
...
// well as a `abxl::StatusBuilder` object populated with the error and
...
namespace abxl::status_macro_internal {
