...
// You can get a well-formed path by using abxl::CleanPath(), and
...
//
// Usage:
// string path = abxl::JoinPath("/cns", dirname, filename);
// string path = abxl::JoinPath(FLAGS_test_srcdir, filename);
//
...
// This is the behavior of the old abxl::JoinPath().
// Arguments must be convertible to absl::string_view.
//
// Usage:
// string path = abxl::JoinPathRespectAbsolute("/cns", dirname, filename);
// string path = abxl::JoinPathRespectAbsolute(FLAGS_test_srcdir, filename);
template <typename... T>
...
// there is no "." in the basename, this is equivalent to abxl::Basename(path).
